    ...
    <div class="toolbar-container">
      ...
      <div class="toolbar-group">
        ...
        <ArButton size="xs" type="ghost" icon title="加粗" :disabled="!hasActiveEditor" @click="execCommand('toggleBold')">
          <template #icon>
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
              <path d="M6 4h6a4 4 0 1 1 0 8H6z" /><path d="M6 12h7a4 4 0 1 1 0 8H6z" />
            </svg>
          </template>
        </ArButton>
        <!-- 斜体 -->
        <ArButton size="xs" type="ghost" icon title="斜体" :disabled="!hasActiveEditor" @click="execCommand('toggleItalic')">
          <template #icon>
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
              <line x1="17" y1="6" x2="10" y2="18" /><line x1="19" y1="6" x2="14" y2="6" /><line x1="10" y1="18" x2="5" y2="18" />
            </svg>
          </template>
        </ArButton>
        <!-- 下划线 -->
        ...
        <ArButton size="xs" type="ghost" icon title="删除线" :disabled="!hasActiveEditor" @click="execCommand('toggleStrike')">
          <template #icon>
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
              <path d="M6 5c0-1 .7-2 2-2h8c1.3 0 2 1 2 2" /><path d="M18 12H6" /><path d="M16 19c0 1-.7 2-2 2H9c-1.3 0-2-1-2-2" />
            </svg>
          </template>
        </ArButton>
        <!-- 行内代码 -->
      ...
      <div class="toolbar-actions">
        <ArButton size="sm" type="secondary" :loading="saving" @click="emit('saveDraft')" title="存草稿" style="gap: 2px">
          <template #icon><SaveOutline /></template>
          存稿
        </ArButton>
        <ArButton size="sm" type="primary" :loading="saving" @click="emit('publish')" title="发布" style="gap: 2px">
